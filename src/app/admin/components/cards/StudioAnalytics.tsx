...
function Chart({ points, label }: { points: { label: string; value: number | null; at?: number }[]; label: string }) {
 ...
 return <div className="studio-chart">
  ...
  <svg viewBox="0 0 640 200" role="img" aria-label={label}>
   ...
   {points.map((p,i)=>{if(p.value==null)return null;const [x,y]=pos(p.value,i);return <g key={i}><circle cx={x} cy={y} r="3" className="studio-dot"/><circle cx={x} cy={y} r="10" fill="transparent" onMouseEnter={()=>setHover(i)} onMouseLeave={()=>setHover(null)} aria-label={`${p.label} : ${num(p.value)}`}><title>{p.label} : {num(p.value)}</title></circle></g>;})}
   <text x="44" y="196">{points[0]?.label}</text><text x="614" y="196" textAnchor="end">{points[points.length-1]?.label}</text>
  </svg>
  <details className="studio-data"><summary>voir les chiffres</summary><div><table><caption>{label}</caption><thead><tr><th scope="col">date du relevé</th><th scope="col">valeur</th></tr></thead><tbody>{points.map((p,i)=><tr key={i}><th scope="row">{p.label}</th><td>{num(p.value)}</td></tr>)}</tbody></table></div></details>
 </div>;
}
...
export default function StudioAnalytics() {
 ...
 const [query,setQuery]=useState(''),[sort,setSort]=useState<'recent'|'views'|'gain'>('recent');
 ...
 const videos=(studio?.videos||[]).filter(v=>(platform==='all'||v.platform===platform)&&normalize(v.title+' '+v.platform).includes(normalize(query.trim()))).sort((a,b)=>(sort==='views'?(b.views??-Infinity)-(a.views??-Infinity):sort==='gain'?(b.gain24h??-Infinity)-(a.gain24h??-Infinity):0)||(b.publishedAt||0)-(a.publishedAt||0));
 ...
 return <Card title="studio" span={12} i={0} className="studio" aside={<button className="studio-refresh" onClick={refresh} disabled={loading}>{loading?'actualisation…':'actualiser ↻'}</button>}>
  <div className="studio-heading"><h3>Vue d’ensemble</h3><button className="studio-jump" onClick={()=>{const el=document.getElementById('studio-publications');el?.scrollIntoView({block:'start'});el?.focus({preventScroll:true});}}>voir les dernières vidéos ↓</button><span title={stamp(studio?.generated||null)}>mise à jour auto · {age(studio?.generated||null)}</span></div>
  ...
  <div className="studio-list-heading"><h3 id="studio-publications" tabIndex={-1}>Publications</h3><span>{videos.length} suivies</span></div>
  <div className="studio-search"><label className="sr-only" htmlFor="studio-search">Chercher une publication</label><input id="studio-search" type="search" placeholder="chercher une vidéo, un sujet…" value={query} onChange={e=>{setQuery(e.target.value);setLimit(8);}}/>{query&&<button onClick={()=>{setQuery('');setLimit(8);}}>effacer</button>}<label className="sr-only" htmlFor="studio-sort">Trier les publications</label><select id="studio-sort" value={sort} onChange={e=>{setSort(e.target.value as typeof sort);setLimit(8);}}><option value="recent">les plus récentes</option><option value="views">les plus vues</option><option value="gain">progression sur 24 h</option></select></div>
  {!videos.length?<p className="studio-chart-empty">{loading?'Chargement des publications…':query?'Aucune publication ne correspond à cette recherche.':platform==='facebook'||platform==='x'?'Les abonnés sont suivis. Les statistiques des publications ne sont pas encore connectées.':'Aucune publication disponible pour ce réseau.'}</p>:<div className="studio-table-wrap"><table className="studio-table"><thead><tr><th scope="col">Contenu</th><th scope="col">Vues</th><th scope="col" title="Variation entre deux relevés espacés de 24 à 27 heures">24 h</th><th scope="col">J’aime</th><th scope="col">Commentaires</th></tr></thead><tbody>{videos.slice(0,limit).map(v=><VideoRow key={v.id} video={v} expanded={expanded===v.id} toggle={()=>setExpanded(expanded===v.id?null:v.id)}/>)}</tbody></table></div>}
