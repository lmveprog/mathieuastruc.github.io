...
- état constaté : vues comparables sur tiktok, youtube et, depuis le 9 septembre, instagram ; pas de mesures de vues Facebook/X dans la base. les données manquantes restent `—`, avec le dernier relevé connu à côté. il faudra une source autorisée de statistiques Facebook/X pour avoir les cinq plateformes.
- instagram : le scraping anonyme est mort (« require_login », même depuis le mac) et le pont mac (launchagent 10 h 30) n'a jamais tourné, bloqué par la protection du dossier Documents. depuis le 9 septembre le lab lit mon compte par l'**api officielle** (`core/sources/instagram_api.py` sur le vps, appelé par `lab-cron.sh`) avec le jeton longue durée du bot instagram dm (`~/assistantcontentcreator/.env`, rafraîchi 2×/mois par son cron) : abonnés, liste des posts et vraies vues (insights). ce jeton ne voit que mon compte : la watchlist instagram de la veille reste muette.
