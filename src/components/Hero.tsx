  ...
  return (
    <main
      style={{
        ...
        padding: "80px clamp(1.25rem, 5vw, 4rem) clamp(3rem, 6vw, 5rem)",
      }}
    >
      {/* ── Header (fades out after first message) ──────── */}
      <div
        style={{
          textAlign: "center",
          marginBottom: hasMessages ? "var(--space-lg)" : "var(--space-xl)",
          transition: `all ${500}ms cubic-bezier(0.16,1,0.3,1)`,
          opacity: hasMessages ? 0.35 : 1,
          transform: hasMessages ? "scale(0.88) translateY(-8px)" : "scale(1)",
      ...
      <div style={{ width: "100%", maxWidth: "660px" }}>
        <div
          style={{
            ...
            transition: `border-color ${120}ms ease, background ${120}ms ease`,
        ...
        {!hasMessages && (
          ...
          >
            {SUGGESTIONS.map((q) => (
              <SuggestionChip key={q} label={q} onSelect={() => send(q)} />
            ))}
          </div>
        )}
      </div>
  ...
  return (
    <button
      ...
      style={{
        ...
        transition: `background ${120}ms ease, border-color ${120}ms ease`,
      }}
    >
      <svg
        width="13"
      ...
      >
        <path
          ...
          stroke={active ? "white" : "var(--color-text-tertiary)"}
          strokeWidth="1.5"
          strokeLinecap="round"
